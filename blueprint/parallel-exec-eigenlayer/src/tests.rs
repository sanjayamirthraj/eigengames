...
use crate::jobs::compute_x_square::CalculateTaskEventHandler;
...
async fn run_eigenlayer_incredible_squaring_test(
    ...
    let x_square_eigen = CalculateTaskEventHandler::new(contract.clone(), eigen_client_context);
...
pub async fn setup_task_spawner(
    ...
    async move {
        loop {
            ...
            if get_receipt(
                task_manager
                    .createNewTask(100u32, quorums.clone())
...
async fn setup_test_context() -> EigenSquareContext {
    let server_address = format!("{}:{}", "127.0.0.1", 8081);
    let api_client = ApiClient::new();
    
    EigenSquareContext {
        client: AggregatorClient::new(&server_address).expect("Failed to create aggregator client"),
        api_client,
...
#[tokio::test]
async fn test_api_calculation() {
    let api_client = ApiClient::new();
    let result = api_client.get_calculation().await.unwrap();
    let zero_bytes: [u8; 32] = [0u8; 32];
    assert!(result.as_slice() != zero_bytes); // Compare byte slices directly
}
