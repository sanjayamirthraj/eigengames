...
const EthereumTransactionBatching = () => {
  ...
  const createMempool = (scene, mempoolRef) => {
    ...
    mempoolGroup.position.set(-15, 0, 0); // Moved closer to center
  ...
  const generateTransactions = (scene, mempoolRef, transactionsRef) => {
    ...
    const mempoolPosition = new THREE.Vector3(-15, 0, 0); // Updated position
  ...
  const runIndividualSimulation = async () => {
    if (!indivSceneRef.current) return;
  ...
  const runBatchSimulation = async () => {
    if (!batchSceneRef.current) return;
    ...
    try {
      ...
      setExplanation(`Batch processing complete: ${transactionsProcessed} transactions in ${(totalTimeBatch/1000).toFixed(1)}s`);
      await new Promise(resolve => setTimeout(resolve, 1500));
    ...
    <div className="flex flex-col w-full mt-auto bg-white text-gray-900 rounded-lg overflow-hidden">
      <div className="p-4 border-b border-gray-200">
        ...
        <p className="text-gray-600 mt-2">
          Side-by-side comparison of traditional fee-based sequential processing vs. optimized state-based batching.
        </p>
      </div>
